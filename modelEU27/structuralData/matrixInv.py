...
def main(n_iter: int = 1000, n: int = 2000, seed: int | None = None) -> None:
    rng = np.random.default_rng(seed)

    t0 = time.perf_counter()
    for i in range(1, n_iter + 1):
        if i % 100 == 0: print(f"[{i}/{n_iter}] genero matrice {n}x{n} e calcolo inversa...", flush=True)
        ...
        if i % 100 == 0: print(f"  -> completato ciclo {i}, checksum={checksum:.6g}", flush=True)
